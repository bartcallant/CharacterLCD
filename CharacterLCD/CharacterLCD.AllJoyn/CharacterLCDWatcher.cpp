...
CharacterLCDWatcher::CharacterLCDWatcher(AllJoynBusAttachment^ busAttachment) :
...
CharacterLCDWatcher::~CharacterLCDWatcher()
{
    UnregisterFromBus();
}

void CharacterLCDWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "Callant.CharacterLCD" };
...
void CharacterLCDWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "Callant.CharacterLCD"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "Callant.CharacterLCD"),
            port);
        Added(this, args);
    }
...
void CharacterLCDWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<CharacterLCDWatcher>
        };
...
void CharacterLCDWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_CharacterLCDIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &CharacterLCDWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void CharacterLCDWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void CharacterLCDWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
